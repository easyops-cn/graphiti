...
| `graphiti_core/prompts/extract_edges.py` | **Prompt增强** | **Edge Reflexion 互斥约束 + 语义等价容忍规则** |
| `graphiti_core/utils/maintenance/edge_operations.py` | **Bug修复** | **wrong_direction 下游实现：交换 source/target entity ID** |
| `graphiti_core/prompts/extract_nodes.py` | **Prompt增强** | **Node Reflexion: entities_to_reclassify 仅在类型实际变更时输出，消除确认性误报** |
| `graphiti_core/prompts/extract_nodes.py` | **Prompt增强** | **实体抽取增加假设性/条件性陈述过滤规则，不抽取不存在的实体** |
| `graphiti_core/prompts/extract_edges.py` | **Prompt增强** | **Edge Reflexion 实体对匹配：按 (source, target) 配对而非语义相似度，修复 facts_to_correct 失效** |
| `graphiti_core/prompts/extract_nodes.py` | **Bug修复** | **Node Reflexion 传入实体当前类型，修复 reclassify 因缺少类型信息无法触发** |
| `graphiti_core/utils/maintenance/node_operations.py` | **Bug修复** | **extract_nodes_reflexion 从传入 name 列表改为 {name, type} 字典列表** |
