...
| `graphiti_core/cross_encoder/qwen_reranker_client.py` | **功能增强** | **QwenRerankerConfig 新增 api_style/batch_size：支持 vLLM rerank 模式部署（/v1/rerank 批量打分），默认 chat 风格保持兼容** |
| `src/elevo_memory/services/entity.py`（主项目） | **性能优化** | **Bulk 写入 embedding 批量化：批前 create_batch 一次生成全批向量，新增 precomputed_embedding 参数，失败回退逐条** |
| `graphiti_core/telemetry/telemetry.py` | **默认值修改** | **遥测默认改为关闭（true→false）：posthog 上报在内网环境不可达时阻塞主流程** |
| `graphiti_core/driver/falkordb_driver.py` | **Bug修复** | **build_fulltext_query：① group_id 不再拼进 fulltext 串（UUID 连字符被分词器拆碎致 BM25 恒 0 命中，改 Cypher 后置过滤）；② 标识符特征查询（IP/≥2 纯数字 token）用 AND 替代 OR** |
| `graphiti_core/search/search_utils.py` + `graph_queries.py` | **性能优化** | **① edge 全文搜索 Cypher 锚定节点 uuid（75s→3.6ms）；② edge cosine 走向量索引 KNN（1.5s→10ms）；③ 新增边/节点向量索引 DDL（存量 group 需手动补建）** |
| `graphiti_core/utils/maintenance/node_operations.py` + `graphiti.py` | **性能优化** | **resolve_extracted_nodes 支持 skip_llm_fallback；add_triplet 启用：Triplet 实体名由调用方显式指定，确定性匹配足够，跳过 LLM 兜底（慢网关 60s 超时 → 8s）；另加零候选短路** |
| `graphiti_core/models/nodes/node_db_queries.py` | **Bug修复** | **FalkorDB 批量保存: 避免 SET n = node 导致 embedding 存储为 List** |

| `graphiti_core/search/search.py` | **Bug修复** | **cross_encoder rerank：summary 为空的节点不参与 rerank（裸名文本判别不可靠，会被错误顶高/压低），改为按 RRF 顺序追加在 rerank 结果之后** |
| `graphiti_core/driver/falkordb_driver.py` | **Bug修复** | **_is_identifier_query 改用原始 query 形态判定（点分≥3段=IP/域名、连字符≥3=长 hostname），修复自然语言短词（how/fix）被误判为标识符** |

| `graphiti_core/search/search.py` | **行为回退** | **无 summary 节点恢复裸名送 cross_encoder（query==name 实测稳定 0.99+；尾部追加方案致无 summary 目标本体被截断丢失）。细分度问题交由 elevo 锚定层处理** |
